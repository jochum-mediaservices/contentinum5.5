...
class Collection extends StorageDirectory
{
    ...
    public function processRequest(array $params = null, $posts = null)
    {
        try {
            $base = $posts['element'];
            $ident = $base['ident'];
            $config = new \Zend\Config\Config(include CON_ROOT_PATH . DS . $this->getEntity()->getCurrentPath() . DS . $base['current'] . '.php', true);
            $config->collections->{$ident}->assets = array();
            foreach ($posts["collection"] as $file) {
                $config->collections->{$ident}->assets[] = $file;
            }
            $writer = new \Zend\Config\Writer\PhpArray();
            $writer->toFile(CON_ROOT_PATH . DS . $this->getEntity()
                ->getCurrentPath() . DS . $base['current'] . '.php', $config);
